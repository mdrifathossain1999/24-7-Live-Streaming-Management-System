...
export default function SettingsManager({ token, isApproved = true, username }: SettingsProps) {
  const isAdmin = username === 'admin' || username === 'diamondvaiteam@gmail.com';
  ...
  const [userApproveLoading, setUserApproveLoading] = useState<number | null>(null);
  ...
  const handleToggleApproval = async (u: AdminUser) => {
    setUserApproveLoading(u.id);
    setUserManagementError('');
    setUserManagementSuccess('');
    
    const newApprovedValue = u.approved === 1 ? 0 : 1;
    
    try {
      const { error: fetchErr } = await safeFetchJson(`/api/users/${u.id}/approve`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}` 
      ...
      if (fetchErr) {
        ...
        const actionMessage = newApprovedValue === 1 ? 'Approved / Access Granted' : 'Disapproved / Access Revoked';
        setUserManagementSuccess(`Successfully ${actionMessage.toLowerCase()} for user "${u.username}"`);
        setUsers(prev => prev.map(usr => usr.id === u.id ? { ...usr, approved: newApprovedValue } : usr));
      }
    } catch (err) {
      setUserManagementError('Failed to update permission status');
    } finally {
      setUserApproveLoading(null);
    }
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 items-center">
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <label
                      htmlFor={isApproved ? "logo-upload" : undefined}
                      className={`px-3.5 py-1.5 bg-slate-950 hover:bg-slate-800 border border-slate-800 text-slate-350 hover:text-white rounded-lg text-xs font-semibold cursor-pointer transition-colors flex items-center gap-1 ${!isApproved ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                    ...
                    {settings?.logoPath && (
                      <button
                        ...
                        disabled={!isApproved}
                        className="px-3.5 py-1.5 border border-slate-800 hover:border-rose-950 hover:bg-rose-950/15 disabled:opacity-40 disabled:cursor-not-allowed text-slate-400 hover:text-rose-400 rounded-lg text-xs font-semibold transition-colors cursor-pointer flex items-center gap-1"
                      >
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 h-fit">
          ...
          <form onSubmit={handleChangePassword} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={!isApproved}
              className="w-full py-2.5 bg-slate-950 border border-slate-800 hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed text-slate-300 hover:text-white font-medium rounded-xl text-xs transition-colors flex items-center justify-center gap-1.5 cursor-pointer mt-2"
            >
      ...
      {isAdmin && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
          ...
          <button
            ...
            disabled={keyGenerateLoading || !isApproved}
            className="px-4 py-2 bg-red-600 hover:bg-red-700 disabled:bg-red-800/55 disabled:cursor-not-allowed text-white text-xs font-semibold rounded-xl transition-colors flex items-center gap-1.5 self-start sm:self-auto cursor-pointer shadow-lg shadow-red-950/15"
          >
            {keyGenerateLoading ? (
                ...
                <tbody className="divide-y divide-slate-800/60 text-white">
                  {users.map((u) => (
                    <tr key={u.id} className="hover:bg-slate-900/40 transition-colors">
                      ...
                      <td className="px-4 py-3 font-semibold break-all max-w-[150px]" title={u.username}>{u.username}</td>
                      <td className="px-4 py-3 text-center">
                        {u.username === 'admin' || u.username === 'diamondvaiteam@gmail.com' ? (
                          <span className="text-[10px] bg-red-950/45 text-red-400 border border-red-900/30 px-2.5 py-1 rounded-full font-semibold uppercase tracking-wider">
                            Super Admin
                          </span>
                        ) : (
                          <button
                            ...
                            disabled={userApproveLoading === u.id || !isApproved}
                            className={`text-[10px] px-2.5 py-1 rounded-full font-semibold uppercase tracking-wider border transition-all cursor-pointer ${
                              u.approved === 1
                                ? 'bg-emerald-950/40 text-emerald-400 border-emerald-900/30 hover:bg-emerald-900/30'
                                : 'bg-amber-950/45 text-amber-500 border-amber-900/30 hover:bg-amber-900/30'
                            } disabled:opacity-50 disabled:cursor-not-allowed`}
                            title={u.approved === 1 ? "Click to Revoke Site Access" : "Click to Grant Site Access"}
                          >
                            {userApproveLoading === u.id ? (
                              <span className="inline-block w-3 h-3 border border-slate-400 border-t-white rounded-full animate-spin"></span>
                            ) : u.approved === 1 ? (
                              'Approved'
                            ) : (
                              'Pending Admin'
                      ...
                      <td className="px-4 py-3 text-right">
                        {u.username === 'admin' || u.username === 'diamondvaiteam@gmail.com' ? (
                          <span className="text-[10px] bg-slate-900 text-slate-500 border border-slate-800 px-2.5 py-1 rounded-full font-semibold">
                            Permanent
                          </span>
                        ) : (
                          <button
                            ...
                            disabled={userDeleteLoading === u.id || !isApproved}
                            className="p-1.5 text-slate-400 hover:text-rose-400 border border-slate-800/80 hover:border-rose-950 hover:bg-rose-950/10 disabled:opacity-45 disabled:cursor-not-allowed rounded-lg transition-colors cursor-pointer"
